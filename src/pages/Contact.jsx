...
export default function Contact() {
  ...
  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText("jaiadithiya11@gmail.com");
      setIsCopied(true);
      setTimeout(() => {
        setIsCopied(false);
      }, 3000);
      ...
      <div className="flex flex-col gap-8 sm:flex-row sm:justify-between sm:gap-0 md:flex-col md:justify-start md:gap-8">
        <div className="space-y-6">
          ...
          <p className="max-w-md text-white opacity-70 sm:max-w-xs md:max-w-md lg:text-lg">
            Feel free to reach out if you're looking to connect, have a question, or want to collaborate on something fun in tech!
          </p>
        </div>
